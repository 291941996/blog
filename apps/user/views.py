	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			if password.isdigit():
				...
				return render(request, 'accounnt/signup.html', context=context)
			if password != password2:
				context['pwd_error'] = 'unequal'
				return render(request, 'accounnt/signup.html', context)
			...
			if pwd_length < 8 or pwd_length > 20:
				...
				return render(request, 'accounnt/signup', context)
		...
		if form.is_valid():
			...
			if user:
				if user.is_active:
					...
					return render(request, 'accounnt/login.html', context)
			else:
				# 比较失败，还在login
				context['error'] = True
				return render(request, 'accounnt/login.html', context)
